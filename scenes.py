...
class Level(GameScene):
    ...
    def __init__(self, display: pygame.Surface, manager, fps=60):
        ...
        self.normal_platforms = Group()
        self.crash_platforms = Group()
    ...
    def spawn_platform(self, x: int, y: int):
        ...
        if random.choice((True, True, True, False)) and 0 <= x < self.size[0] - 65:
            if random.choice([1, 2, 2, 2, 3, 3, 2]) == 1:
                platform = Platform(
                    x, y, "assets/platforms/platform_crash.webp", self.size)
                self.crash_platforms.add(platform)
            elif random.choice([1, 2, 2, 2, 3, 3, 2]) == 3:
                platform = Platform(
                    x, y, "assets/platforms/moving_platform.webp", self.size)
                self.moving_platforms.add(platform)
            else:
                platform = Platform(
                    x, y, "assets/platforms/platform.png", self.size)
                self.normal_platforms.add(platform)
    ...
    def check_collisions(self):
        ...
        for group in (self.normal_platforms, self.items, self.enemies, self.crash_platforms, self.moving_platforms):
            for coll in group.get_collisions(self.main_character, ignore):
                if coll in self.crash_platforms and self.main_character.falling == True:
                    self.crash_platforms.remove(coll)
    ...
    def redraw(self, win):
        ...
        self.normal_platforms.draw(win)
        self.crash_platforms.draw(win)
        self.moving_platforms.draw(win)
    ...
    def handle_movement(self):
        ...
        if self.scroll_down:
            ...
            self.normal_platforms.update(self.offset)
            self.crash_platforms.update(self.offset)
            self.moving_platforms.update(self.offset, moving=True)
    ...
    def restart(self):
        ...
        self.normal_platforms.clear()
        self.crash_platforms.clear()
...
class GameOverMenu(GameScene):
    ...
    def update_highscore(self, new_highscore):
        ...
        if self.score != -1 and new_highscore > self.score:
    ...
    def update_money(self, money_to_add):
        if self.get_game_value(self.MONEY_KEY) != -1:
            self.set_game_value(
                self.MONEY_KEY, self.money_collected + money_to_add)
            self.money_collected = money_to_add

    def show_revive_dialog(self):
        """метод для показа диалога возрождения"""
        if self.get_game_value(self.MONEY_KEY) != -1 and self.get_game_value(self.MONEY_KEY) >= self.revive_price:
...
class PauseMenu(GameOverMenu, Level):
